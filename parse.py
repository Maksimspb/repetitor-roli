...
# ---- Пьесы: каждая со своим каноном имён и путями --------------------------
# Волки лежат на прежнем месте (data/), новые пьесы — в plays/<slug>/.
# Канон: ключ = каноничное имя; значения = варианты написания (в нижнем регистре).
PLAY_CONFIGS = {
    "volki": {
        "title": "Волки и овцы (расширенная версия)",
        "author": "А. Н. Островский",
        "dir": ROOT / "data",                       # play.docx, play.json, scene_notes.json
        "canon": {
            ...
            "Беркутов":    ["беркутов"],
            "Чугунов":     ["чугунов"],
            # Две Глафиры — разные актрисы, роли репетируются порознь.
    ...
    "gadanie": {
        "title": "Станешь гадать как девку отдать",
        "author": "Гульнара Соколова (по Островскому)",
        "dir": ROOT / "plays" / "gadanie",
        # В исходнике список действующих лиц («ЛИЦА») продублирован под заголовками
        # «Действие первое/второе» и мешает разбору. Отрезаем всё до реального начала —
        # ремарки, с которой открывается первое действие.
        "real_start": "Комната в доме Пузатова",
        "canon": {
            "Устинья Наумовна":     ["устинья наумовна", "устинья"],
            "Антип Антипыч":        ["антип антипыч", "антип"],
            "Матрена Савишна":      ["матрена савишна", "матрёна савишна"],
            "Марья Антиповна":      ["марья антиповна"],
            "Степанида Трофимовна": ["степанида трофимовна", "степанида"],
            "Ширялов":              ["ширялов", "парамон ферапонтыч"],
            "Дарья":                ["дарья"],
            "Аграфена Кондратьевна":["аграфена кондратьевна", "аграфена"],
            "Липочка":              ["липочка", "олимпиада самсоновна", "олимпиада"],
            "Фоминишна":            ["фоминишна"],
        },
        "manual": {},
    },
...
_ap.add_argument("--play", default="volki", choices=list(PLAY_CONFIGS))
_ARGS, _ = _ap.parse_known_args()
_CFG = PLAY_CONFIGS[_ARGS.play]

PLAY_SLUG = _ARGS.play
TITLE = _CFG["title"]
AUTHOR = _CFG["author"]
DOCX = _CFG["dir"] / "play.docx"
OUT = _CFG["dir"] / "play.json"
NOTES = _CFG["dir"] / "scene_notes.json"
CANON = _CFG["canon"]
...
def parse():
    ...
    # отрезать преамбулу (титул + дублированный список действующих лиц), если задан
    # маркер реального начала: берём ближайший заголовок «Действие…» перед ним.
    real_start = _CFG.get("real_start")
    if real_start:
        ri = next((i for i, p in enumerate(paras) if real_start in p), None)
        if ri is not None:
            aj = max((j for j in range(ri) if ROMAN_ACT.match(paras[j])), default=0)
            paras = paras[aj:]
    ...
    def ensure_scene():
        """Действие без «явлений» (в некоторых пьесах 1-е действие идёт сплошняком):
        перед первой репликой создаём одну сцену на весь акт, иначе она не попадёт
        в список сцен приложения (оно строит список из scene-юнитов)."""
        nonlocal scene
        if scene is None:
            scene = "вся сцена"
            units.append({"type": "scene", "title": scene, "act": act, "synthetic": True})
